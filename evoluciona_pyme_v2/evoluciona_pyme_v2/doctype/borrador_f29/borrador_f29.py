...
class Borrador_F29(Document):
    ...
    def generar_html_f29(self):
        ...
        # =====================================================
        if not self.tabla_debitos and not self.tabla_creditos and not self.tabla_impuestos:
        ...
        # Calcular débitos
        if self.tabla_debitos:
            for linea in self.tabla_debitos:
        ...
        if self.tabla_creditos:
            for linea in self.tabla_creditos:
                monto = float(linea.monto or 0)
        ...
        if self.tabla_impuestos:
            for linea in self.tabla_impuestos:
                monto = float(linea.monto or 0)
                if linea.tipo_operacion_subtotal == 'Resta':
        ...
        postergacion_periodo = float(self.postergacion_del_periodo or 0)
        total_pagar = max(0, iva_determinado - postergacion_periodo + subtotal_impuestos)    
        
        # Actualizar campos del documento
        self.subtotal_debitos = subtotal_debitos
        self.subtotal_creditos = subtotal_creditos
        self.subtotal_postergacion_iva = subtotal_postergacion
        self.subtotal_otros_impuestos = subtotal_impuestos
        self.impuesto_determinado = iva_determinado
        self.remanente_mes_siguiente = remanente
        self.total_a_pagar_f29 = total_pagar
        ...
        if self.tabla_debitos:
            for linea in self.tabla_debitos:
                ...
                link_docs = ""
                if es_calculado:
                    link_docs = generar_link_documentos(linea.codigo_f29, self.cliente, self.ano, self.mes)
